...
describe PodmanContainer do
  ...
  let(:os_mock) { mock("os") }
  let(:container_mock) { mock("containers") }
  ...
  before do
    ...
    podman_mock.stubs(:containers).returns(container_mock)
    container_mock.stubs(:where).returns(File.read("test/fixtures/podman-ps-a"))
