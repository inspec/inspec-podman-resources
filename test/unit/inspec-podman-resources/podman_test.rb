...
describe Podman do
  ...
  before do
    ...
    # info
    inspec_mock.stubs(:command).with("podman info --format json").returns(mock("podmand_info").tap do |cmd|
      cmd.stubs(:exit_status).returns(0)
      cmd.stubs(:stdout).returns(File.read("test/fixtures/podman-info"))
      cmd.stubs(:stderr).returns("")
    end)
    # version
    inspec_mock.stubs(:command).with("podman version --format json").returns(mock("podman_version").tap do |cmd|
      cmd.stubs(:exit_status).returns(0)
      cmd.stubs(:stdout).returns(File.read("test/fixtures/podman-version"))
      cmd.stubs(:stderr).returns("")
    end)
    ...
    # inspect
    inspec_mock.stubs(:command).with("podman inspect 591270d8d80d --format json").returns(mock("podmand_inspect").tap do |cmd|
      cmd.stubs(:exit_status).returns(0)
